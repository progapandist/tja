...
//	verb|t or f (separable)|official meaning|colloquial meaning|example|aux override
//
...
func load() []*Stem {
	...
	for _, line := range strings.Split(raw, "\n") {
		...
		s := stems[len(stems)-1]
		v := Verb{Name: f[0], Sep: f[1] == "t", Official: f[2], Colloquial: f[3], Example: f[4], Stem: s}
		if len(f) > 5 {
			v.Aux = f[5]
		}
